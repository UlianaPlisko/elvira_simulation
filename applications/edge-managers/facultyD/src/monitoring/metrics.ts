...
const CONTAINER_NAME = CONFIG.metrics?.containerName || 'facultyD-edge';
...
// ======================= METRICS (prefixed facultyD_) =======================
const lambdaGauge          = new promClient.Gauge({ name: 'facultyD_load_lambda',               help: 'λ(t) — итоговая нагрузка 0-1 (facultyD)' });
const powerWattsGauge      = new promClient.Gauge({ name: 'facultyD_power_watts',               help: 'Текущая мощность (Вт) (facultyD)' });
const energyTotalKwh       = new promClient.Counter({ name: 'facultyD_energy_kwh',              help: 'Общее потребление энергии (kWh) (facultyD)' });
const transitionsTotal     = new promClient.Counter({ name: 'facultyD_transitions_total',       help: 'Количество переходов через порог (facultyD)' });

// Host (node-exporter)
const hostCpuPercent       = new promClient.Gauge({ name: 'facultyD_host_cpu_percent',          help: 'CPU хоста (%) — node-exporter (facultyD)' });

// Container (logporter / docker metrics)
const containerCpuPercent  = new promClient.Gauge({ name: 'facultyD_cpu_load',                  help: 'CPU контейнера (%) — как в docker stats (facultyD)' });
const containerMemBytes    = new promClient.Gauge({ name: 'facultyD_mem_usage_bytes',           help: 'RAM контейнера (bytes) (facultyD)' });
const containerMemPercent  = new promClient.Gauge({ name: 'facultyD_mem_load',                  help: 'RAM контейнера (%) (facultyD)' });
const containerNetRxBytes  = new promClient.Gauge({ name: 'facultyD_net_rx_bytes_per_sec',      help: 'Сеть входящая (bytes/sec) (facultyD)' });
const containerNetTxBytes  = new promClient.Gauge({ name: 'facultyD_net_tx_bytes_per_sec',      help: 'Сеть исходящая (bytes/sec) (facultyD)' });
const containerDiskRead    = new promClient.Gauge({ name: 'facultyD_disk_read_bytes_per_sec',   help: 'Диск чтение (bytes/sec) (facultyD)' });
const containerDiskWrite   = new promClient.Gauge({ name: 'facultyD_disk_write_bytes_per_sec',  help: 'Диск запись (bytes/sec) (facultyD)' });
const containerPids        = new promClient.Gauge({ name: 'facultyD_process_count',             help: 'Количество процессов в контейнере (facultyD)' });

// Nginx (nginx-exporter)
const nginxConnections     = new promClient.Gauge({ name: 'facultyD_nginx_connections_active',  help: 'Активные соединения Nginx (facultyD)' });
const nginxRequestsTotal   = new promClient.Gauge({ name: 'facultyD_requests_total',            help: 'Всего запросов (R) (facultyD)' });
const nginxRps             = new promClient.Gauge({ name: 'facultyD_requests_per_second',       help: 'RPS за последнюю минуту (facultyD)' });

// Books disk / cache (local scan)
const booksUsedBytes       = new promClient.Gauge({ name: 'facultyD_books_used_bytes',          help: 'Занято в cache/books (bytes) (facultyD)' });
const booksUsedMb          = new promClient.Gauge({ name: 'facultyD_books_used_mb',             help: 'Занято в cache/books (MB) (facultyD)' });
const booksUtilPercent     = new promClient.Gauge({ name: 'facultyD_books_util_percent',        help: 'Заполненность cache/books (%) (facultyD)' });
...
export async function updateMetrics() {
  try {
    ...
    // 7. Nginx
    const connections = await query('nginx_connections_active{job="nginx-facultyD"}');
    const requestsTotal = await query('sum(nginx_http_requests_total{job="nginx-facultyD"})');
    const rps = await query('rate(nginx_http_requests_total{job="nginx-facultyD"}[1m])');
    ...
    if (lambda > (CONFIG.load?.threshold ?? 0.6) && previousLambda <= (CONFIG.load?.threshold ?? 0.6)) {
      ...
      console.log(`⚡ facultyD: edge-activated (λ=${lambda.toFixed(3)}), +${alphaKwh.toFixed(6)} kWh`);
    }
    previousLambda = lambda;

  } catch (err) {
    console.error('facultyD updateMetrics error:', err);
  }
}
...
export async function resetMetrics() {
  try {
    ...
    previousLambda = 0;
    console.log('Все метрики facultyD сброшены');
  } catch (err) {
    console.error('facultyD: error while resetting metrics', err);
    throw err;
  }
}
export { register };
