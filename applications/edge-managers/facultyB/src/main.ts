// applications/facultyB-manager/src/main.ts
...
const CONTROL_PORT = process.env.CONTROL_PORT ? parseInt(process.env.CONTROL_PORT) : 3002;
const PROMETHEUS_URL = CONFIG.prometheus?.url || 'http://prometheus:9090';

let baselineRequestsFacultyB = 0;
let zeroRpsUntil = 0;
...
app.post('/reset-metrics', async (req, res) => {
  try {
    ...
    try {
      const total = await promScalar('sum(nginx_http_requests_total{job="nginx-facultyB"})');
      baselineRequestsFacultyB = total;
      zeroRpsUntil = Date.now() + 5000;
      console.log('FacultyB: baselineRequestsFacultyB =', baselineRequestsFacultyB);
    } catch (err) {
      console.warn('FacultyB: failed to set baselineRequestsFacultyB:', err);
    }

    res.json({ result: 'ok', msg: 'facultyB metrics reset' });
  } catch (e: any) {
    console.error('facultyB reset-metrics error:', e);
...
app.get('/facultyB-metrics', async (_req, res) => {
  try {
    const [
      eTotal,
      powerWatts,
      ...
      promScalar('facultyB_energy_kwh'),
      promScalar('facultyB_power_watts'),
      promScalar('facultyB_load_lambda'),
      promScalar('facultyB_host_cpu_percent'),
      promScalar('facultyB_cpu_load'), // container CPU %
      promScalar('facultyB_mem_usage_bytes'),
      promScalar('facultyB_mem_load'),
      promScalar(`rate(docker_network_received_bytes{containerName="${CONFIG.metrics?.containerName || 'facultyB-nginx'}"}[1m])`),
      promScalar(`rate(docker_network_transmit_bytes{containerName="${CONFIG.metrics?.containerName || 'facultyB-nginx'}"}[1m])`),
      promScalar(`rate(docker_io_read_bytes{containerName="${CONFIG.metrics?.containerName || 'facultyB-nginx'}"}[1m])`),
      promScalar(`rate(docker_io_write_bytes{containerName="${CONFIG.metrics?.containerName || 'facultyB-nginx'}"}[1m])`),
      promScalar(`docker_process_pids_count{containerName="${CONFIG.metrics?.containerName || 'facultyB-nginx'}"}`),
      promScalar('facultyB_nginx_connections_active'),
      promScalar('facultyB_requests_total'), // Gauge from metrics server
      promScalar('facultyB_requests_per_second'),
      promScalar('facultyB_books_used_bytes'),
      promScalar('facultyB_books_used_mb'),
      promScalar('facultyB_books_util_percent'),
      promScalar('facultyB_transitions_total')
    ]);

    // Protect against immediate noise after reset
    const rps = Date.now() < zeroRpsUntil ? 0 : Number(rpsRaw.toFixed(2));
    const requestsSinceReset = Math.max(0, requestsTotalRaw - baselineRequestsFacultyB);
    ...
    console.error('/facultyB-metrics error:', e?.message || e);
    res.status(500).json({ error: 'failed to fetch facultyB metrics' });
  }
});
...
app.get('/prom-query', async (req, res) => {
  try {
    ...
    console.error('Prom proxy error (facultyB):', e);
...
app.post('/trigger-prefetch', async (req, res) => {
  console.log('Prefetch triggered (facultyB) opts=', req.body || {});
...
const server = app.listen(CONTROL_PORT, () => {
  ...
  console.log(`   → http://localhost:${CONTROL_PORT}/facultyB-metrics — all metrics JSON`);
});
